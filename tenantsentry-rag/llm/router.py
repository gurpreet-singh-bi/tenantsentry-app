...
def analyse_clause(
    ...
    system_prompt = "\n".join([
        ...
        '      "confidence": "confirmed|probable|flag",',
        '      "legislation_ref": "...",',
        ...
        "",
        "AQ-NEW-11 — CONFIDENCE TIER (mandatory for every flag):",
        "  'confirmed' — The clause TEXT makes this risk explicit and unambiguous. You can cite the exact",
        "    wording. No interpretation required. Example: lease says 'Tenant must return premises to base",
        "    building condition' — make-good risk is confirmed.",
        "  'probable' — Strong indicators are present in the text but the risk requires professional",
        "    interpretation to confirm. Example: outgoings clause mentions 'all costs incurred' without",
        "    a capital works carve-out — probable that CapEx is recoverable but clause is ambiguous.",
        "  'flag'     — Possible risk that may or may not materialise depending on facts not visible in",
        "    the text alone (e.g. practice, quantum, counterparty behaviour). Example: market rent review",
        "    mechanism does not state the valuation methodology — flag for advisor review.",
        "  RULES: (1) Never omit confidence — always set one of the three values.",
        "  (2) Most HIGH/VOID flags should be 'confirmed' (the clause is explicit).",
        "  (3) Use 'probable' when the risk is real but depends on interpretation.",
        "  (4) Use 'flag' sparingly — only when you genuinely cannot determine risk from the text alone.",
        "  (5) Confidence is INDEPENDENT of severity — a 'flag' tier HIGH risk is still HIGH.",
    ])
