...
# Process-level cache — keyed by jurisdiction code
...
def get_cpi_snapshot(jurisdiction: str, num_quarters: int = 8) -> dict:
    """
    Return the last `num_quarters` quarters of All Groups CPI for `jurisdiction`.

    Result is cached in-process — the ABS API is only called once per
    jurisdiction per process start, regardless of how many CPI clauses
    are found in the lease.

    Returns a snapshot dict (see module docstring for schema).
    """
    jur = jurisdiction.upper()

    if jur in _snapshot_cache:
        return _snapshot_cache[jur]
